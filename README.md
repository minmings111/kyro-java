...
이 경험을 통해 Kyro에는 더 명시적인 책임 분리가 필요하다고 판단했습니다. Java의 접근 제어자, 인터페이스, record, 패키지 구조, 생성자 기반 의존성 주입을 활용하면 어떤 객체가 어떤 책임을 갖는지, 어떤 값이 어느 계층까지 전달되는지, 모듈 간 계약이 어디서 강제되는지를 코드 구조 안에 더 분명하게 남길 수 있다고 보았습니다.

이번 Java 전환에서는 단순 포팅보다 유지보수 가능한 백엔드 구조를 먼저 잡는 데 집중했습니다.
